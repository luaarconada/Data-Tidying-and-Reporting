...
flights_dbi <- tbl(con, "flights")
airports_dbi <- tbl(con, "airports")

# Group by destination, summarize by summing the number of flights, arrange them in descending number of flights.
# # Join with the dataset 'airports_dbi' and rename the variable 'airport_name' to 'name'
# This is a pipeline
flights_dbi %>%
  group_by(dest) %>%
  summarize(num_flights = n()) %>%
  arrange(desc(num_flights)) %>%
  inner_join(airports_dbi, by = c("dest" = "faa")) %>%
