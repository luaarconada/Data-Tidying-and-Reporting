...
knn_classifier <- function(test_img, k_value) {
  ...
  closest_digit <- train_nist$digit[train_sample][order(dists)[1:k_value]] # Use input value for k
...
# Define confidence level functions
knn_confidence <- function(test_img, k_value) {
  dists <- apply(train_nist$px[train_sample, ], 1, function(train_img) sqrt(sum((train_img - test_img)^2)))
  closest_digits <- train_nist$digit[train_sample][order(dists)[1:k_value]] # Select the k closest digits
  majority_digit <- as.numeric(names(sort(table(closest_digits), decreasing = TRUE)[1])) # Get the digit with the highest frequency
  confidence <- sum(closest_digits == majority_digit) / k_value # Calculate confidence level
  return(confidence)
}
...
ui <- fluidPage(
  fluidRow(
    column(width = 10, offset = 1,
           h3("Prediction of digits", align = "center", style = "font-weight: bold; font-size: 20px;"),
  ...
  fluidRow(
    ...
    column(width = 6,
           hr(),
           h5("Classified Digit:", style = "font-size: 16px;"),
           verbatimTextOutput("result"),
           hr(),
           h5("Confidence:", style = "font-size: 16px;"),
...
server <- function(input, output, session) {
  ...
  output$input_image <- renderPlot({
    ...
    par(mar = c(1, 1, 1, 1))
    show_digit(c(255 * t(img)), main = "Preview of the input digit")
    hover <- input$hover
    if (!is.null(hover)) {
      x <- floor(hover$x)
      y <- floor(hover$y)
      text(x, y, paste("x:", x, ", y:", y), adj = c(0, 0))
    }
  }, height = 400)
  
  # Render classifier explanation based on selected classifier
  output$classifier_explanation <- renderUI({
    req(input$classifier)
    ...
    tagList(
      p(explanation, style = "font-size: 16px;")
    )
  })
  ...
  observeEvent(input$classify, {
    if (input$classifier == "Select Classifier") { # Check if the default option is selected
      showNotification("Please select a classifier.", duration = 5, type = "warning")
      return()
    }
    
    if (is.null(input$file1$datapath)) {
      showNotification("No file selected. Please choose an image file.", duration = 5, type = "warning")
      return()
    }
    
    if (file.info(input$file1$datapath)$size > 10e6) {
      showNotification("File size limit exceeded. Please choose an image file smaller than 10MB.", duration = 5, type = "warning")
      return()
    }
    
    tryCatch({
      img <- readPNG(input$file1$datapath)
      test_img <- c(255 * t(img))
      
      if (input$classifier == 'Decision Tree') {
        digit <- dt_classifier(test_img, train_sample(), input$cp_value)
        ...
        digit <- knn_classifier(test_img, input$k_value)
        output$result <- renderText({
          paste("Predicted Digit:", digit)
        })
      } else {
        digit <- classifier(test_img)
        output$result <- renderText({
          paste("Predicted Digit:", digit)
        })
      }
      ...
      showNotification("Error processing the image. Please try again with a valid image file.", duration = 5, type = "error")
    })
  })
  ...
  observeEvent(input$file1, {
    ...
    output$confidence <- renderText({ NULL })
  })
